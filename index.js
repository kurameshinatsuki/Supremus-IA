...
prettyLog(msg); const text = extractText(msg); if (!text) return; if (msg.key.fromMe) { cacheBotReply && cacheBotReply(msg.key.remoteJid, text); return; } const remoteJid = msg.key.remoteJid; const isGroup = remoteJid.endsWith('@g.us'); const pushName = msg.pushName || msg.notifyName || null; const contextInfo = msg.message?.extendedTextMessage?.contextInfo; const quotedText = contextInfo?.quotedMessage ? extractTextFromQuoted(contextInfo) : null; const quotedSender = extractQuotedSender(contextInfo); let quotedMessageInfo = quotedText && quotedSender ? { sender: quotedSender, text: quotedText } : null; const senderJid = msg.key.participant || remoteJid; const mentionedJids = contextInfo?.mentionedJid || []; const botNumber = '111536592965872'; const isMentioned = mentionedJids.some(jid => jid.includes(botNumber)) || (text && text.includes('@' + botNumber)) || (text && text.toLowerCase().includes('supremia')); const isCommand = text.startsWith('/'); const shouldReply = !isGroup || isCommand || quotedMessageInfo || isMentioned; if (!shouldReply) return; try { if (isCommand) { const [cmd, ...args] = text.slice(1).trim().split(/\s+/); const reply = await handleCommand(cmd, args, msg, sock); if (reply) { await sendReply(sock, msg, { text: reply }); cacheBotReply && cacheBotReply(remoteJid, reply); return; } } const replyObj = await nazunaReply(text, senderJid, remoteJid, pushName, isGroup, quotedMessageInfo); if (!replyObj || !replyObj.text) return; if (DEBUG) console.log('🧠 Nazuna replyObj:', JSON.stringify(replyObj, null, 2)); const conversion = convertReplyMentionsToClickable(replyObj.text, replyObj.mentions || []); const finalText = conversion.finalText; const mentionJids = conversion.mentionJids; await sendReply(sock, msg, mentionJids.length > 0 ? { text: finalText, mentions: mentionJids } : { text: finalText }); cacheBotReply && cacheBotReply(remoteJid, finalText); } catch (error) { console.error('❌ Erreur lors du traitement du message:', error?.stack || error); await sendReply(sock, msg, { text: '❌ Désolé, une erreur est survenue. Veuillez réessayer plus tard.' }); } 
